...
const findReleases = async ({
  ...
  try {
    latestRelease = await context.octokit.repos.getLatestRelease(context.repo())
  } catch (error) {
    if (error.status === 404) {
      log({ context, message: `No latest release found` })
    } else {
      throw error
    }
  }
  const latestReleasePublishedAt = latestRelease.data?.published_at || ''
  ...
  const sortedSelectedReleases = sortReleases(
    filteredReleases.filter(
      (r) =>
        !r.draft &&
        (!r.prerelease || includePreReleases) &&
        (!r.published_at || r.published_at >= latestReleasePublishedAt)
    ),
    tagPrefix
  )
